...
internal static class EntranceTeleportPatches
{
    ...
    /// Prevent spectators from broadcasting that they're opening the main entrance doors
    /// </summary>
    [HarmonyPatch(typeof(EntranceTeleport), nameof(EntranceTeleport.FinishOpeningEntrance))]
    [HarmonyTranspiler]
    private static IEnumerable<CodeInstruction> SpectatorDisableDoorClose(IEnumerable<CodeInstruction> instructions)
    {
        return new CodeMatcher(instructions)
            .MatchForward(false,
                new CodeMatch(OpCodes.Call,
                    Method(typeof(EntranceTeleport), nameof(EntranceTeleport.SyncFinishOpeningEntranceRpc))))
            .Set(OpCodes.Call, ((Action<EntranceTeleport>)SyncDoorIfNotDead).Method)
            .InstructionEnumeration();
        ...
        static void SyncDoorIfNotDead(EntranceTeleport entrance)
        {
            ...
            entrance.SyncFinishOpeningEntranceRpc();
        }
    }
}